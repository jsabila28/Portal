...
if(isset($_GET['empno']) && (in_array($_GET['empno'], explode(",", $pachecklist)) || $_GET['empno'] == $user_empno)){
	$paf_empno 	= $_GET['empno'];
	if($user_empno != $paf_empno){
...
if(isset($_GET['id']) && $_GET['id'] != ''){
	...
	if(get_assign("pa","viewall",$user_empno)){
		...
		$sql1 = $hr_pdo->prepare("SELECT * FROM tbl_paf_sji LEFT JOIN tbl201_jobinfo ON ji_empno = paf_empno WHERE paf_id = ? AND (paf_empno = ? OR FIND_IN_SET(paf_empno, ?) > 0)");
		$sql1->execute([ $_GET['id'], $user_empno, $pachecklist ]);
	}
...
<style type="text/css">
	...
	#tbl_qty tbody td, #tbl_qty input, #tbl_qty textarea {
		...
		<div class="panel-body">
			...
			<form class="form-horizontal" id="form_pa">
					...
					<div class="form-group">
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<?php }else{ ?>
									<div class="col-md-8">
										<select id="paf_job" class="form-control selectpicker" data-width="fit" data-live-search="true" title="Select" required>
											<?php
												// foreach ($hr_pdo->query("SELECT DISTINCT jd_code, jd_title FROM tbl_paqty_setting a JOIN tbl_jobdescription b ON b.`jd_code` = a.`qtyset_for`") as $r1) {
												foreach ($hr_pdo->query("SELECT DISTINCT jd_code, jd_title FROM tbl201_jobrec a JOIN tbl_jobdescription b ON b.`jd_code` = a.`jrec_position` WHERE LOWER(a.jrec_status) = 'primary' AND a.jrec_department = 'SLS'") as $r1) {
													echo "<option value=\"" . $r1['jd_code'] . "\">" . $r1['jd_title'] . "</option>";
												}
											?>
										</select>
									</div>
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<?php }else if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
										<div class="col-md-8">
							...
							<div class="form-group">
								...
								<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
									<div class="col-md-9">
				...
				<div class="pull-right">
					<?php 
							if($user_empno == $paf_ratedby || $user_empno == $paf_empno){
								if($paf_id != ''){ ?>
			...
			<div style="overflow: auto; max-width: 100%;">
				<table id="tbl_qty" style="width: 99.9%;">
					<thead>
						<tr>
							<?=($user_empno == $paf_ratedby || $user_empno == $paf_empno ? "<th class='tddelqty' style='display: none;'></th>" : "")?>
							<th></th>
							<th></th>
						...
						<tr>
							<?=($user_empno == $paf_ratedby || $user_empno == $paf_empno ? "<th class='tddelqty' style='display: none;'></th>" : "")?>
							<th style="min-width: 300px; text-align: center;">KEY PERORMANCE INDICATOR</th>
							<th style="min-width: 100px; text-align: center;">TARGET</th>
							<th style="min-width: 150px; text-align: center;">1</th>
							<th style="min-width: 150px; text-align: center;">2</th>
							<th style="min-width: 150px; text-align: center;">3</th>
							<th style="min-width: 150px; text-align: center;">4</th>
							<th style="min-width: 100px; text-align: center;">WEIGHT</th>
							<th style="min-width: 100px; text-align: center;">ATTAINMENT</th>
							<th style="min-width: 70px; text-align: center;">RATING</th>
							<th style="min-width: 70px; text-align: center;">SCORE</th>
						</tr>
					</thead>
					<tbody>
						<?php
							...
							if(count($arrqty) > 0){
								...
								if($user_empno == $paf_ratedby || $user_empno == $paf_empno){
									...
									foreach ($arrqty as $r1) {
										echo "<tr qtyid='" . $r1['paqty_id'] . "' class='qtyrows'>";
										echo "<td class='tddelqty' style='display: none;'><button class='btn btn-sm btn-danger btndelqty'><i class='fa fa-times'></i></button></td>";
								  		echo "<td style='height: 1px; width: 300px;'><textarea style='height: 70px; vertical-align: middle;' class='qty-kpi' disabled>" . $r1['paqty_kpi'] . "</textarea></td>";
										echo "<td style='height: 1px; width: 100px;'><input style='height: 100%;' type='text' class='qty-target' value='" . $r1['paqty_target'] . "' disabled></td>";
										...
										echo "<td style='height: 1px;' class='qtyscale1' scalemin='" . $scale_arrval[0] . "' scalemax='" . $scale_arrval[1] . "'><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale1-min' min='0' max='100' value='" . $scale_arrval[0] . "' disabled><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale1-max' min='0' max='100' value='" . $scale_arrval[1] . "' disabled></td>";
										
										$scale_arr = explode(",", $r1['paqty_scale2']);
										$scale_arrval[0] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[0])[0];
										$scale_arrval[1] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[1])[0];
										echo "<td style='height: 1px;' class='qtyscale2' scalemin='" . $scale_arrval[0] . "' scalemax='" . $scale_arrval[1] . "'><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale2-min' min='0' max='100' value='" . $scale_arrval[0] . "' disabled><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale2-max' min='0' max='100' value='" . $scale_arrval[1] . "' disabled></td>";

										$scale_arr = explode(",", $r1['paqty_scale3']);
										$scale_arrval[0] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[0])[0];
										$scale_arrval[1] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[1])[0];
										echo "<td style='height: 1px;' class='qtyscale3' scalemin='" . $scale_arrval[0] . "' scalemax='" . $scale_arrval[1] . "'><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale3-min' min='0' max='100' value='" . $scale_arrval[0] . "' disabled><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale3-max' min='0' max='100' value='" . $scale_arrval[1] . "' disabled></td>";

										$scale_arr = explode(",", $r1['paqty_scale4']);
										$scale_arrval[0] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[0])[0];
										$scale_arrval[1] = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[1])[0];
										echo "<td style='height: 1px;' class='qtyscale4' scalemin='" . $scale_arrval[0] . "' scalemax='" . $scale_arrval[1] . "'><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale4-min' min='0' max='100' value='" . $scale_arrval[0] . "' disabled><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale4-max' min='0' max='100' value='" . $scale_arrval[1] . "' disabled></td>";

										$weightval = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$r1['paqty_weight'])[0];
										echo "<td style='height: 1px; width: 100px;' class='addpercent qtyweight' weightval='" . $weightval . "'><input style='height: 100%; width: 80%;' type='number' class='qty-weight' min='0' max='100' value='" . $weightval . "' disabled></td>";

										$percentdisp = preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$scale_arr[0]);
										$percentdisp = isset($percentdisp[1]) ? $percentdisp[1] : "";
										echo "<td style='height: 1px; width: 100px;'><input style='height: 100%; display: inline-block; width: 85%;' type='number' class='qtyattainment' min='0' max='100' value=\"" . preg_split('/(?<=[0-9])(?=[a-z%]+)/i',$r1['paqty_attainment'])[0] . "\" disabled><span style='display: inline-block; width: 15%;'>$percentdisp</span></td>";

										echo "<td style='width: 70px;'><input type=\"number\" class=\"qtyrating\" min=\"1\" max=\"4\" value=\"" . $r1['paqty_rating'] . "\" disabled></td>";
							...
							echo "<tr id='qtyfinalscore'>";
							echo "<td class='scorepadding' colspan='9' style='border: none;'></td>";
				...
				<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
				<br>
				<button class="btn btn-primary" id="btnaddqty" style="display: none; margin-bottom: 10px;"><i class="fa fa-plus"></i></button>
				<?php } ?>
			</div>
			<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
				<br>
			...
			<div style="overflow-y: auto; max-width: 100%;">
				<table id="tbl_qlty" <?=!($user_empno == $paf_ratedby || $user_empno == $paf_empno) ? "style=\"width: 99.9%;\"" : ""?>>
					<thead>
						<tr>
							<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
								<th class="tddelqlty" style="width: 20px; display: none;"></th>
								<th style="max-width: 300px; text-align: center;">Core Competencies/Values</th>
								<th style="max-width: 300px; text-align: center;">Definition</th>
								<th style="max-width: 440px; width: 440px; text-align: center;">Behavioral Indicators</th>
								<th style="max-width: 100px; width: 100px; text-align: center;">( ✖ ) or ( ✔ )</th>
								<th style="max-width: 300px; width: 300px; text-align: center;">Remarks</th>
							<?php }else{ ?>
								<th style="max-width: 300px; text-align: center;">Core Competencies/Values</th>
								<th style="max-width: 300px; text-align: center;">Definition</th>
								<th style="max-width: 400px; width: 400px; text-align: center;">Behavioral Indicators</th>
								<th style="max-width: 100px; width: 100px; text-align: center;">( ✖ ) or ( ✔ )</th>
								<th style="max-width: 300px; width: 300px; text-align: center;">Remarks</th>
							<?php } ?>
						</tr>
							...
							if(count($arrqlty) > 0){
								...
								if($user_empno == $paf_ratedby || $user_empno == $paf_empno){
									foreach ($arrqlty as $r1) {
										...
										echo "<tr qltyid='" . $r1['paqlty_id'] . "' class='qltyrows'>";
								  		echo "<td class='tddelqlty' style='display: none;'><button class='btn btn-sm btn-danger btndelqlty'><i class='fa fa-times'></i></button></td>";
								  		echo "<td style='height: 1px; min-width: 300px; width: 300px;'><textarea style='height: 70px; vertical-align: middle;' class='qlty-competencies' disabled>" . $r1['paqlty_competencies'] . "</textarea></td>";
								  		echo "<td style='height: 1px; min-width: 300px; width: 100%;'><textarea style='height: 70px; vertical-align: middle;' class='qlty-definition' disabled>" . $r1['paqlty_definition'] . "</textarea></td>";

										echo "<td colspan='3' style='padding: 0; text-align: center;'>";
										echo "<table class='qltyindicatortbl' style='width: 840px; min-width: 840px; height: 100%;'>";
										foreach ($indicators as $rk2 => $r2) {
											echo "<tr class='qltyindicatorrow'>";
								  			echo "<td style='width: 40px; display: none;'><button class='btn btn-sm btn-default btndelqltyindicator'><i class='fa fa-times'></i></button></td>";
											echo "<td style='height: 1px; min-width: 400px; width: 100%;'>";
											echo "<input style='height: 100%; text-align: left;' type='text' class='qlty-indicator' value='" . $r2 . "' disabled>";
											echo "</td>";
											echo "<td style='min-width: 50px; width: 50px; display: none;'>";
											echo "<button class='qltychecker qltychk1 " . ($check[$rk2] == 0 ? "qltychecked" : "") . "' value='0'>✖</button>";
											echo "</td>";
											echo "<td style='min-width: 50px; width: 50px; display: none;'>";
											echo "<button class='qltychecker qltychk2 " . ($check[$rk2] == 1 ? "qltychecked" : "") . "' value='1'>✔</button>";
											echo "</td>";
											echo "<td style='min-width: 100px; width: 100px; text-align: center;'><span class='spanqlty'>" . ($check[$rk2] == 1 ? "✔" : "✖") . "</span></td>";

											echo "<td style='min-width: 300px; width: 300px;'>";
											echo "<input type='text' class='form-control qltyremarks' style='text-align: left; display: none;' value=\"" . $remarks1[$rk2] . "\">";
											echo "<span style=''>" . $remarks1[$rk2] . "</span>";
											echo "</td>";
											echo "</tr>";
										}
										echo "</table>";
										echo "<button class='btn btn-sm btn-default btnaddqltyindicator' style='display: none; margin: 5px; width: 80%;'>ADD</button>";
										echo "</td>";
									...
									foreach ($arrqlty as $r1) {
										...
										echo "<tr>";
										echo "<td style=\"max-width: 300px;\">" . $r1['paqlty_competencies'] . "</td>";
										echo "<td style=\"max-width: 300px;\">" . $r1['paqlty_definition'] . "</td>";

										echo "<td colspan='3' style='padding: 0;'>";
										echo "<table class='qltyindicatortbl' style='width: 800px; min-width: 800px; height: 100%;'>";
										foreach ($indicators as $rk2 => $r2) {

											echo "<tr>";
											echo "<td style='min-width: 400px;  width: 100%;'>";
											echo "<label>" . $r2 . "</label>";
											echo "</td>";
											echo "<td style='min-width: 100px; text-align: center;'>" . ($check[$rk2] == 1 ? "✔" : "✖") . "</td>";
				...
				<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
				<br>
				<button class="btn btn-primary" id="btnaddqlty" style="display: none; margin-bottom: 10px;"><i class="fa fa-plus"></i></button>
				<?php } ?>
			</div>
			<?php if($user_empno == $paf_ratedby || $user_empno == $paf_empno){ ?>
			<br>
...
<script type="text/javascript">
	$(function(){
		...
		$('textarea').on('keyup keypress', function() {
	  	...
	  	$("#tbl_qlty").on("click", ".qltychecker", function(){
	  		$(this).addClass("qltychecked");
	  		$(this).parent().parent().find(".qltychecker").not(this).removeClass("qltychecked");
	  	});

	  	$("#tbl_qty").on("input", ".qtyattainment", function(){
	  		var rating1 = "";
	  		const val1 = $(this).val().replace(/%/g, '');
	  		if(val1 >= parseFloat($(this).parents("tr").find(".qtyscale4").find(".qty-scale4-min").val())){
	  			rating1 = 4;
	  		}else if(val1 >= parseFloat($(this).parents("tr").find(".qtyscale3").find(".qty-scale3-min").val())){
	  			rating1 = 3;
	  		}else if(val1 >= parseFloat($(this).parents("tr").find(".qtyscale2").find(".qty-scale2-min").val())){
	  			rating1 = 2;
	  		}else if(val1 <= parseFloat($(this).parents("tr").find(".qtyscale1").find(".qty-scale1-max").val())){
	  			rating1 = 1;
	  		}

	  		$(this).parents("tr").find(".qtyrating").val(rating1);
	  		if(rating1){
	  			$(this).parents("tr").find(".qtyscore").text( roundto2(($(this).parents("tr").find(".qty-weight").val()/100) * $(this).parents("tr").find(".qtyrating").val()) );
	  			var finalscore = 0;
	  			$(".qtyscore").filter(function(){ return $(this).text() != ''; }).each(function(){
	  				finalscore += parseFloat($(this).text());
	  			});
	  	...
	  	$("#btnqtyedit").click(function(){
	  		...
	  		$("#tbl_qty input, #tbl_qty textarea").attr("disabled", false);
			$("#tbl_qty .tddelqty").show();
			$("#btnaddqty").show();
			$("#tbl_qty .scorepadding").attr("colspan", "10");
	  	});
	  	...
	  	$("#btnqtysave").click(function(){
	  		...
	  		var arrqty = [];
	  		$("#tbl_qty tbody tr.qtyrows").each(function(){
	  			arrqty.push({ 
	  							"id": $(this).attr("qtyid"),
	  							"kpi": $(this).find(".qty-kpi").val(),
	  							"target": $(this).find(".qty-target").val(),
	  							"scale1": $(this).find(".qty-scale1-min").val()+","+$(this).find(".qty-scale1-max").val(),
	  							"scale2": $(this).find(".qty-scale2-min").val()+","+$(this).find(".qty-scale2-max").val(),
	  							"scale3": $(this).find(".qty-scale3-min").val()+","+$(this).find(".qty-scale3-max").val(),
	  							"scale4": $(this).find(".qty-scale4-min").val()+","+$(this).find(".qty-scale4-max").val(),
	  							"weight": $(this).find(".qty-weight").val(),
	  							"attainment": $(this).find(".qtyattainment").val(),
	  							"rating": $(this).find(".qtyrating").val(),
	  							"act": $(this).attr("qtyid").includes("tmp") ? "add" : "update"
	  						});
	  		});
  			...
  			function(data1){
				...
				if(data1.status == 1){
					...
					$("#btnqtyedit").show();
	  				...
	  				$("#tbl_qty input, #tbl_qty textarea").attr("disabled", true);
	  				$("#tbl_qty .tddelqty").hide();
	  				$("#btnaddqty").hide();
					$("#tbl_qty .scorepadding").attr("colspan", "9");

	  				for(x in data1["newid"]){
	  					$("[qtyid='"+x+"']").attr("qtyid", data1["newid"][x]);
	  				}
				}else{
					alert("Failed to save. Please try again");
				}
	  	...
	  	$("#btnaddqty").click(function(){
	  		var txt1 = "<tr qtyid='tmp"+tmpcnt+"' class='qtyrows'>";
	  		txt1 += "<td class='tddelqty'><button class='btn btn-sm btn-danger btndelqty'><i class='fa fa-times'></i></button></td>";
	  		txt1 += "<td style='height: 1px; min-width: 300px; width: 300px;'><textarea style='height: 70px; vertical-align: middle;' class='qty-kpi'></textarea></td>";
	  		txt1 += "<td style='height: 1px; min-width: 100px; width: 100px;'><input style='height: 100%;' type='text' class='qty-target'></td>";
	  		txt1 += "<td style='height: 1px;' class='qtyscale1' scalemin='' scalemax=''><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale1-min' min='0' max='100'><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale1-max' min='0' max='100'></td>";
	  		txt1 += "<td style='height: 1px;' class='qtyscale2' scalemin='' scalemax=''><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale2-min' min='0' max='100'><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale2-max' min='0' max='100'></td>";
	  		txt1 += "<td style='height: 1px;' class='qtyscale3' scalemin='' scalemax=''><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale3-min' min='0' max='100'><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale3-max' min='0' max='100'></td>";
	  		txt1 += "<td style='height: 1px;' class='qtyscale4' scalemin='' scalemax=''><input style='height: 100%; width: 45%;' placeholder='MIN' type='number' class='qty-scale4-min' min='0' max='100'><span style='margin: 3px; display: inline-block;'>-</span><input style='height: 100%; width: 45%;' type='number' placeholder='MAX' class='qty-scale4-max' min='0' max='100'></td>";
	  		txt1 += "<td style='height: 1px; width: 100px;' class='addpercent qtyweight'><input style='height: 100%; width: 80%;' type='number' class='qty-weight' min='0' max='100'></td>";
	  		txt1 += "<td style='height: 1px; width: 100px;'><input style='height: 100%;' type='number' class='qtyattainment' min='0' max='100'></td>";
	  		txt1 += "<td style='width: 100px;'><input type=\"number\" class=\"qtyrating\" min=\"1\" max=\"4\" value=\"\" disabled></td>";
	  		txt1 += "<td class='qtyscore'></td>";
	  		txt1 += "</tr>";

	  		tmpcnt++;

	  		$("#qtyfinalscore").before(txt1);
	  	});

	  	$("#tbl_qty").on("click", ".btndelqty", function(){
	  		$(this).parents("tr").remove();
	  	});

	  	$("#tbl_qty").on("input", ".qty-kpi", function(){
	  	...
	  	$("#btnqltyedit").click(function(){
	  		...
	  		$("#tbl_qlty input, #tbl_qlty textarea").attr("disabled", false);
	  		$("#tbl_qlty .btndelqltyindicator").parent().show();
	  		$("#tbl_qlty .tddelqlty, #tbl_qlty .btnaddqltyindicator").show();
	  		$("#btnaddqlty").show();
	  	});
	  	...
	  	$("#btnqltysave").click(function(){
	  		...
	  		var arrqlty = [];
	  		$("#tbl_qlty tbody tr.qltyrows").each(function(){
	  			var arrindicator = [];
	  			$(this).find("table tr").each(function(){
	  				arrindicator.push({ 
	  					"indicator": $(this).find(".qlty-indicator").val(),
	  					"check": ($(this).find(".qltychecked").length > 0 ? $(this).find(".qltychecked").val() : 0),
	  					"remarks": $(this).find(".qltyremarks").val()
	  				});
	  			});
	  			arrqlty.push({ 
					"id": $(this).attr("qltyid"),
					"competencies": $(this).find(".qlty-competencies").val(),
					"definition": $(this).find(".qlty-definition").val(),
					"indicators": arrindicator,
					"act": $(this).attr("qltyid").includes("tmp") ? "add" : "update"
				});
	  		});
  			...
  			function(data1){
				...
				if(data1.status == 1){
					...
					$("#tbl_qlty button").attr("disabled", true);
			  		...
			  		$("#tbl_qlty input, #tbl_qlty textarea").attr("disabled", true);
			  		$("#tbl_qlty .btndelqltyindicator").parent().hide();
			  		$("#tbl_qlty .tddelqty, #tbl_qlty .btnaddqltyindicator").hide();
			  		$("#btnaddqlty").hide();
			  		...
			  		for(x in data1["newid"]){
	  					$("[qltyid='"+x+"']").attr("qltyid", data1["newid"][x]);
	  				}
				}else{
					alert("Failed to save. Please try again");
				}
	  	...
	  	$("#btnaddqlty").click(function(){
	  		var txt1 = "<tr qltyid='tmp"+tmpcnt2+"' class='qltyrows'>";
	  		txt1 += "<td class='tddelqlty'><button class='btn btn-sm btn-danger btndelqlty'><i class='fa fa-times'></i></button></td>";
	  		txt1 += "<td style='height: 1px; min-width: 300px; width: 300px;'><textarea style='height: 70px; vertical-align: middle;' class='qlty-competencies'></textarea></td>";
	  		txt1 += "<td style='height: 1px; min-width: 300px; width: 100%;'><textarea style='height: 70px; vertical-align: middle;' class='qlty-definition'></textarea></td>";

			txt1 += "<td colspan='3' style='padding: 0; text-align: center;'>";
			txt1 += "<table class='qltyindicatortbl' style='width: 840px; min-width: 840px; height: 100%;'></table>";
			txt1 += "<button class='btn btn-sm btn-default btnaddqltyindicator' style='margin: 5px; width: 80%;'>ADD</button>";
			txt1 += "</td>";
			...
			txt1 += "</tr>";
	  		...
	  		$("#tbl_qlty > tbody").append(txt1);
	  	});

	  	$("#tbl_qlty").on("click", ".btnaddqltyindicator", function(){
	  		var txt1 = "<tr class='qltyindicatorrow'>";
	  			txt1 += "<td style='width: 40px;'><button class='btn btn-sm btn-default btndelqltyindicator'><i class='fa fa-times'></i></button></td>";
				txt1 += "<td style='height: 1px; min-width: 400px; width: 100%;'>";
				txt1 += "<input style='height: 100%; text-align: left;' type='text' class='qlty-indicator'>";
				txt1 += "</td>";
				txt1 += "<td style='min-width: 50px; width: 50px;'>";
				txt1 += "<button class='qltychecker qltychk1' value='0'>✖</button>";
				txt1 += "</td>";
				txt1 += "<td style='min-width: 50px; width: 50px;'>";
				txt1 += "<button class='qltychecker qltychk2 qltychecked' value='1'>✔</button>";
				txt1 += "</td>";
				txt1 += "<td style='min-width: 100px; width: 100px; text-align: center; display: none;'><span class='spanqlty'></span></td>";

				txt1 += "<td style='min-width: 300px; width: 300px;'>";
				txt1 += "<input type='text' class='form-control qltyremarks' style='text-align: left;' value=\"\">";
				txt1 += "<span style='display: none;'></span>";
				txt1 += "</td>";
				txt1 += "</tr>";

	  		$(this).parent().find(".qltyindicatortbl").append(txt1);
	  	});

	  	$("#tbl_qlty").on("click", ".btndelqlty", function(){
	  		$(this).parents("tr").remove();
	  	});

	  	$("#tbl_qlty").on("click", ".btndelqltyindicator", function(){
	  		$(this).parents(".qltyindicatorrow").remove();
	  	});

	  	$("#tbl_qlty").on("input", "textarea", function(){
	  		$(this).val(this.value.replace(/\n/g, ''));
	  	});
