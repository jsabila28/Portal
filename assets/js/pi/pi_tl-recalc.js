function fetchTLData_recalc() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                updateTLPIdata_recalc(data, month); // Pass month to the function
                if(data.length > 0){
                    $('#div-tl-pi-recalc').show();
                }
            } else {
    ...
    xhr.open('GET', 'tlpi?month=' + month + '&r=1', true); // PHP to get data
    xhr.send();
}

// Function to update the table with fetched data
function updateTLPIdata_recalc(data, month) {
    let tlTablebody = document.getElementById('tlPiTablebody-recalc');
    ...
    data.forEach(function (item) {
        ...
        checkbox.checked = item.tl_dist_status === '1';

        checkbox.addEventListener('change', function () {
            let newStatus = this.checked ? 1 : 0;
            updateStatus_recalc(item.id, newStatus);
        });
        ...
        input.id = 'retro_' + item.id + '-recalc';
        // input.setAttribute('readonly', 'true');
        input.setAttribute('data-toggle', 'modal');
        input.setAttribute('data-target', '#modal-tl-pi-recalc');
        input.setAttribute('data-tl-id', item.id);
        input.setAttribute('data-previous-value', input.value);
        input.setAttribute('data-tl-name', item.tl_name);

        ar.appendChild(input);
        row.appendChild(ar);

        let grandTotal = document.createElement('td');
        grandTotal.className = 'tl-gtotal';
    ...
    totalSumCell.id = 'totalSumCell-recalc';
...
function updateTotalSum_recalc(totalSum) {
    document.getElementById('totalSumCell-recalc').textContent = totalSum.toLocaleString('en-US');
}


function updateStatus_recalc(id, newStatus) {
    var xhr = new XMLHttpRequest();
    ...
    xhr.send('id=' + encodeURIComponent(id) + '&tl_dist_status=' + encodeURIComponent(newStatus) + '&r=1');
}
