function fetchPIDataRecalculate() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                updatePiTableRecalculate(data, month);
            } else {
                console.error('Failed to fetch data:', xhr.status);
            }
        }
    };
    xhr.open('GET', 'totalpijs?month=' + month + '&r=1', true); //php to get data
    xhr.send();
}

function updatePiTableRecalculate(data, month) {
    let totalPiTableheader = document.getElementById('totalPiTableheader-recalc');
    let totalPiTablebody = document.getElementById('totalPiTablebody-recalc');
    ...
    // FOR ADDING 0.25%
    row17.innerHTML += `<td><b><input type='checkbox' id='additional-recalc' onchange='toggleAdditionalRowsRecalculate()'> Add (0.25%) </b></td>`;
    ...
    h_row2.id = 'h_row2-recalc';
    row18.id = 'row18-recalc';
    row19.id = 'row19-recalc';
    row20.id = 'row20-recalc';
    row21.id = 'row21-recalc';
    row22.id = 'row22-recalc';
    ...
    if (checkForMonth) {
        document.getElementById('additional-recalc').checked = true;
    } else {
        // HIDE 0.25% ROW
        document.getElementById('row18-recalc').style.display = 'none';
        document.getElementById('row19-recalc').style.display = 'none';
        document.getElementById('row20-recalc').style.display = 'none';
        document.getElementById('row21-recalc').style.display = 'none';
        document.getElementById('row22-recalc').style.display = 'none';

    }
}

function toggleAdditionalRowsRecalculate() {
    let displayStyle = document.getElementById('additional-recalc').checked ? '' : 'none';

    let row18 = document.getElementById('row18-recalc');
    let row19 = document.getElementById('row19-recalc');
    let row20 = document.getElementById('row20-recalc');
    let row21 = document.getElementById('row21-recalc');
    let row22 = document.getElementById('row22-recalc');
    ...
    if (document.getElementById('additional-recalc').checked) {
        if (confirm("Do you want to save the 0.25% incentive?")) {
            saveAdditionalRowsRecalculate();
        }
    }
}

function saveAdditionalRowsRecalculate() {
    var month = document.getElementById('pi-month').value;
    let table = document.getElementById('totalPiTablebody-recalc');
    let headers = document.getElementById('h_row2-recalc').children;
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'saveCI?month=' + month + '&r=1', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                let checkbox = document.getElementById('additional-recalc');
